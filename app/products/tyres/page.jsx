...
export default function TyresPage() {
  ...
  useEffect(() => {
    const fetchTyreProducts = async () => {
      try {
        setLoading(true);
        const response = await getProducts();
        if (response.success) {
          // Filter only tyre products
          const tyreProducts = response.data.filter(product => product.category === 'tyre');
          setProducts(tyreProducts);
          // Set initial displayed products
          setDisplayedProducts(tyreProducts.slice(0, productsPerPage));
          
          // Extract unique subcategories from products
          const uniqueSubCategories = [...new Set(tyreProducts.map(product => product.subcategory))];
            ...
            <div className="flex-1">
              ...
              <div className="pb-24">
                {loading ? (
                  ...
                  <div className="text-center py-20">
                    <p className="text-gray-600">No tire products found in this category.</p>
                  </div>
                ) : (
                  <>
